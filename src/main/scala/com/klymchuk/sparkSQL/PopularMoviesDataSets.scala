...
object PopularMoviesDataSets {
  ...
  def loadMovieNames() : Map[Int, String] = {
    ...
    val lines = Source.fromFile("inputData/u.item").getLines()
    for (line <- lines) {
  ...
  def main(args: Array[String]) {
    ...
    val lines = spark.sparkContext.textFile("inputData/u.data").map(x => Movie(x.split("\t")(1).toInt))
